...
The host builds a durable substrate: filesystem receipts under `<state-dir>/receipts` and a filesystem world-model under `<state-dir>/world-models`. The reactor self-mounts and runs a boot cold-miss sweep, so a restart resumes from durable state rather than re-ingesting the backlog.
