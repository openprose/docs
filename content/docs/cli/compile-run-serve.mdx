...
description: The three core verbs in depth, the content-addressed compile cache, and the durable daemon's seven HTTP routes.
---
...
| `GET /status` | The cost rollup plus node count and queue depth. | `{ reactor, nodes, queueDepth, cost }` |
| `GET /cost` | The cost rollup (the headline observability). | `{ reactor, ...costRollup }` |
| `GET /topology` | The node ids only -- a thin id list, not the wired DAG. | `{ reactor, nodes: NodeId[] }` |
| `GET /receipts` | The full ledger receipt stream. | `{ reactor, receipts: Receipt[] }` |
...
<Cards>
  <Card href="/sdk/run" title="The SDK run boundary" description="compileProject and runProject -- the model-bearing functions serve and run drive under the hood." />
  <Card href="/cli/observability" title="Keyless observability" description="Read the receipts, cost, and topology these verbs leave behind, with no key present." />
</Cards>
