...
This walks the full path: `reactor init` to scaffold a project, `reactor compile` to freeze it, then `reactor serve` to drive the scaffold's static gateway to a real receipt.
...
`@openprose/reactor` is the SDK engine the CLI drives. As of this release the SDK is a real dependency of `@openprose/reactor-cli`, so `npm i -g @openprose/reactor-cli` already pulls it in -- the line above just makes the SDK and the replay viewer (`@openprose/reactor-devtools`) explicit so the keyless replay in [Reactor DevTools](/reactor-devtools/quickstart) is there too. None of these pull a model provider or a key.
