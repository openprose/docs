...
<Cards>
  ...
  <Card
    ...
    description="The central teaching: author a truth to keep current, not a sequence of steps to run. Why a contract is a typed function where a bare prompt is `any`."
  />
  <Card
    title="Contracts"
    href="/contracts"
...
`### Maintains` is the load-bearing section: the schema of the truth this contract keeps current. [Contracts](/contracts) teaches it in depth, along with the five kinds and the rest of the authored surface.

How a host *serves* that contract set (memoization, receipts, a reconciler) is the host's concern, not the language's. The contract is the public artifact; it runs unchanged on any compliant host. See [Harness-agnostic](/harness-agnostic) for the seam.
