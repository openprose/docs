...
The `surprise-cause` line counts receipts by what woke them; the cost rollup below it is the actual token spend, where `fresh` is what each surprise cost and `reused` is what memoization saved. That is "cost scales with surprise" -- checkable, with no key and no spend.
